...
// int havdler_two_page(pte_t * pte)
// {
...
void usertrap(void)
{
  ...
  {
    // printf("Page Fault so in Trap.c\n");
    // Virtual address of this pg
    uint64 address = r_stval();
    // this address should not be zero
    if (!address)
    {
      p->killed = 1;
    }
    // handle pages with fault ie make them writable and make cpy also and chane PTE
